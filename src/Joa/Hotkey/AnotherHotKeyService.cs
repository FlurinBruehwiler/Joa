...
public class AnotherHotKeyService : IDisposable
{
    ...
    public AnotherHotKeyService(IJoaLogger joaLogger)
    {
    ...
    {
        int status;
        while ((status = HotKeyHelper.GetMessage(out var msg, nint.Zero, 0, 0)) != 0)
        {
            RegisterHotkeys();
            UnregisterHotKeys();
            ...
            if (msg.Message != HotKeyHelper.WmHotkey)
                continue;
    ...
    private void UnregisterHotKeys()
    {
        ...
        {
            if (!HotKeyHelper.UnregisterHotKey(nint.Zero, id))
            {
    ...
    private void RegisterHotkeys()
    {
        foreach (var (id, hotKey) in _hotKeysToRegister)
        {
            ...
            if (!HotKeyHelper.RegisterHotKey(nint.Zero, id, modifiers, (uint)hotKey.Key))
            {
